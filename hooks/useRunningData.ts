...
export function useRunningData() {
    ...
    const getFeelingLabel = (feeling: 'great' | 'good' | 'average' | 'bad') => {
        switch (feeling) {
    ...
    const generateCSV = () => {
        ...
        const rows = filteredSessions.map(session => [
            ...
            session.pace !== undefined ? session.pace.toString().replace('.', ',') : '',
            session.calories !== undefined ? session.calories.toString() : '',
            session.elevationGain !== undefined ? session.elevationGain.toString() : '',
    ...
    const exportToCSV = async () => {
        try {
            ...
            if (!csvContent) {
                Alert.alert('Aucune donnée', 'Aucune session ne correspond aux critères de filtre');
                setIsExporting(false);
                return;
            }

            // Générer un nom de fichier avec la date
            const fileName = `running_data_${format(new Date(), 'yyyyMMdd_HHmmss')}.csv`;
    ...
    const getRunningStats = () => {
        ...
        const positiveFeeling = sessions.filter(s => s.feeling === 'great' || s.feeling === 'good').length;
        const positivePercentage = Math.round((positiveFeeling / totalSessions) * 100);

        // Trouver la course la plus longue
        const longestRun = [...sessions].sort((a, b) => (b.distance || 0) - (a.distance || 0))[0];
